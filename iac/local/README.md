...
### Seed DynamoDB
- local: `python -m src.shared.infra.repositories.load_user_mock_to_dynamo --target local`
- AWS DEV/HOMOLOG (after cdk deploy): `python -m src.shared.infra.repositories.load_user_mock_to_dynamo --target aws` (set STAGE + Dynamo env vars; blocked on PROD)
